...
import { add, add2Number, addNoCheck } from "../src/simple";
...
const tests = [
    ...
    {
        inputs: [100000000000000000, 100000000000000000],
        expected: 200000000000000000,
    },
...
tests.forEach((config) => {
    test(`Adding ${config.inputs[0]} and ${config.inputs[1]} should equal ${config.expected}`, () => {
        expect(add2Number.apply(this, config.inputs)).toEqual(config.expected);
    });
});
